...
Now, several contestants will participate in this contest. Suppose a contestant solves `x_i` (`0<= x_i<= a_i`) subtasks of the `i`-th problem; his score on the `i`-th problem will be `x_i * frac{100}{a_i}`. The total score of the contestant in the contest is the sum of the scores on all problems, i.e., `sumlimits_{i=1}^{n} x_i * frac{100}{a_i}`.
...
To prove that the contest is a truly *wonderful* one, you have to check whether it is possible to achieve every integer total score from `0` to `100* n` (inclusive). Formally, you have to determine whether the following statement holds:
 ...
 - For every integer `k` where `0<= k<= 100* n`, there exists an array `x` of length `n` (`0<= x_i<= a_i`) such that `k=sumlimits_{i=1}^{n} x_i * frac{100}{a_i}`.
...
In the first test case, for every integer `k` (`0 <= k <= 200`), it is possible to achieve a total score of exactly `k`. For example, when `k=10`, a contestant who passes `0` subtasks in the first problem and `2` subtasks in the second problem achieves a total score of `0 * frac{100}{100} + 2 * frac{100}{20} = 10`.

In the second test case, when `k=95`, it can be proven that it is impossible to achieve a total score of exactly `95`.

In the third test case, for every integer `k` (`0 <= k <= 300`), it is possible to achieve a total score of exactly `k`. For example, when `k=233`, a contestant who passes `25`, `83`, and `25` subtasks in the three problems, respectively, achieves a total score of `25 * frac{100}{50} + 83 * frac{100}{100} + 25 * frac{100}{25} = 233`.
